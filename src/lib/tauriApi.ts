...
export async function currentWindowWorkArea(): Promise<MonitorWorkArea | null> {
  ...
  // Prefer the monitor that currently hosts the pet window so multi-monitor
  // setups clamp and place the chat panel against the correct work area.
  if (isTauriRuntime()) {
    try {
      const monitor = await currentMonitor();
      if (monitor) {
        const area = monitor.workArea;
        const monitorScale = monitor.scaleFactor > 0 ? monitor.scaleFactor : scaleFactor;
        // Tauri reports workArea in physical pixels; convert to logical to match
        // get_pet_window_frame / move_pet_window coordinates.
        return {
          x: Math.round(area.position.x / monitorScale),
          y: Math.round(area.position.y / monitorScale),
          width: Math.round(area.size.width / monitorScale),
          height: Math.round(area.size.height / monitorScale),
          scaleFactor: monitorScale,
        };
      }
    } catch {
      // Fall through to browser screen metrics.
    }
  }
